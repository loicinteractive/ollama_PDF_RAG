    ...
    if use_sample:
        ...
        if file_upload:
            if len(file_upload) > 1:
                selected_file = col1.selectbox(
                    "Select a file to process ↓", 
                    [file.name for file in file_upload],
                    key="file_select"
                )
                # 选择文件后处理该文件
                selected_file_obj = next(file for file in file_upload if file.name == selected_file)
                if st.session_state["vector_db"] is None:
                    with st.spinner(f"Processing selected file: {selected_file}..."):
                        st.session_state["vector_db"] = create_vector_db([selected_file_obj])
                        pdf_pages = extract_all_pages_as_images(selected_file_obj)
                        st.session_state["pdf_pages"] = pdf_pages
            else:
                # 如果只上传了一个文件，直接处理
